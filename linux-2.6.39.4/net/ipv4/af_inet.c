...
lookup_protocol:
    ...
    // 如果使用tcp的话，此时sock->type为SOCK_STREAM,
    // 因此 answer->ops 为 inet_stream_ops
	sock->ops = answer->ops;
	...
	err = -ENOBUFS;
    // 如果是使用tcp的话,sk->prot实际是 tcp_prot
		 ...
		 size_t size, int flags)
{
    ...
    // 如果是TCP的话，sk->sk_prot为 tcp_prot,
    // 即下面实际调用了 tcp_proto.tcp_recvmsg
